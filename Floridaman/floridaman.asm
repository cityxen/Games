...
*=mem_deadline_sprites "CityXenSprites"
#import "cityxensprites.asm"
...
sub_initialize_vars:
	...
	lda #$00
	sta var_player_dir
	sta var_player_anim_fr
	sta var_player_anim_tmr
	rts
...
set_filename_loop1:
	...
	adc #$30
	sta var_filename
	PrintStrAtColor(19,5,var_filename,RED)
	PrintStrAtColor(5,7,"filename:",WHITE)
	PrintStrAtColor(14,7,var_filename,RED)
	Disk_Load_MemName(var_filename,$50,$00)
	PrintStrAtColor(5,9,level_message,BLUE) // show the loaded level message
	// READ JOYSTICK 2, CHECK FOR FIRE BUTTON PRESS
...
sub_move_player:
	...
	rts
ok_move:
	// Assume standing this frame; set direction from joystick below
	lda #$00
	sta var_player_dir
...
move_y:
	...
	beq up
down:
	lda #$03                   // direction: down (overrides horizontal)
	sta var_player_dir
 	inc var_player_y
 	jmp check_boundry
up:
	lda #$04                   // direction: up (overrides horizontal)
	sta var_player_dir
	dec var_player_y
check_boundry:
	lda var_player_x
...
xitckbn:
	...
	lda var_player_y
	sta SPRITE_0_Y
	jsr sub_animate_player     // update player sprite frame
	rts
...
//////////////////////////////////////////////////////////////////////////////////////
sub_update_hud:
	lda VIC_RASTER_COUNTER     // only update near vblank to avoid screen tearing
	cmp #$f8
	bcc hud_exit
	PrintStrAtColor(30,0,"lives:",WHITE)
	PrintDecAtColor(37,0,var_lives,WHITE)
	PrintStrAtColor(30,1,"score:",WHITE)
	PrintDecAtColor(37,1,var_score,WHITE)
hud_exit:
    rts
...
sub_copy_floridaman_font:
	...
	ldx #$00
copy_fmf_loopz1:
	lda mem_floridaman_font,x
	sta $3800,x
	lda mem_floridaman_font+$100,x
	sta $3900,x	
	lda mem_floridaman_font+$200,x
	sta $3A00,x
	lda mem_floridaman_font+$300,x
	sta $3B00,x
	lda mem_floridaman_font+$400,x
	sta $3C00,x
	lda mem_floridaman_font+$500,x
	sta $3D00,x
	lda mem_floridaman_font+$600,x
	sta $3E00,x
	lda mem_floridaman_font+$700,x
	sta $3F00,x
	inx
	bne copy_fmf_loopz1
	rts
...
sub_copy_deadline_sprites:
	...
	ldx #$00
copyloopz1:
	lda mem_deadline_sprites,x
	sta $2000,x
	inx
	bne copyloopz1
copyloopz2:
	lda mem_deadline_sprites+$100,x
	sta $2100,x
	inx
	cpx #$C1
	bne copyloopz2
...
//////////////////////////////////////////////////////////////////////////////////////
// Subroutine: Animate Player Sprite
// Reads var_player_dir; sets SPRITE_0_POINTER accordingly.
// Called from sub_move_player every throttled frame.
//////////////////////////////////////////////////////////////////////////////////////
sub_animate_player:
	lda var_player_dir
	bne anim_is_moving
	// Standing — freeze at frame 0 and show standing sprite
	lda #$00
	sta var_player_anim_fr
	sta var_player_anim_tmr
	lda #SPRITE_FLORIDA_MAN_STANDING
	sta SPRITE_0_POINTER
	rts

anim_is_moving:
	// Advance animation timer; step frame on overflow
	clc
	lda var_player_anim_tmr
	adc #$20
	sta var_player_anim_tmr
	bcc anim_set_sprite
	// Timer overflowed: advance frame 0->1->2->3->0
	lda var_player_anim_fr
	clc
	adc #$01
	and #$03
	sta var_player_anim_fr
...
anim_set_sprite:
	// Select base sprite pointer for this direction
	lda var_player_dir
	cmp #$01
	beq anim_dir_right
	cmp #$02
	beq anim_dir_right        // left has no separate sprites; reuse right frames
	cmp #$03
	beq anim_dir_down
	// direction 4 = up
	lda #SPRITE_FLORIDA_MAN_WALK_UP_1
	jmp anim_apply_frame
anim_dir_right:
	lda #SPRITE_FLORIDA_MAN_WALK_RIGHT_1
	jmp anim_apply_frame
anim_dir_down:
	lda #SPRITE_FLORIDA_MAN_WALK_DOWN_1
anim_apply_frame:
	// Pointer = base + frame index (sprites are consecutive: _1, _2, _3, _4)
	clc
	adc var_player_anim_fr
	sta SPRITE_0_POINTER
	rts

// rainbowtable:.byte YELLOW,GREEN,BLUE,PURPLE,RED,ORANGE
anim_table_squirrel_run:
.byte SPRITE_SQUIRREL_RUN_1,SPRITE_SQUIRREL_RUN_2,0

//////////////////////////////////////////////////////////////////////////////////////
// Joystick reader (inlined from old Joysticks.asm)
// var_joy_2_x: 0=center, $01=right, $FF=left
// var_joy_2_y: 0=center, $01=down,  $FF=up
// var_joy_2_fire: 0=not pressed, $01=pressed
//////////////////////////////////////////////////////////////////////////////////////
var_joy_2_x:             .byte 0
var_joy_2_y:             .byte 0
var_joy_2_fire:          .byte 0
var_joy_2_fire_released: .byte 0
